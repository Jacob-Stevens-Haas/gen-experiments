...
def gen_trajectory(times: np.ndarray, proc_noise: float) -> Trajectory:
    ...
    traj = np.concatenate(
        (np.zeros((1,2)), np.cumsum(d_traj, axis=0)),
        axis = 0
    )
...
def kalman_matrices(times: np.ndarray, meas_ind: np.ndarray | None = None) -> tuple[
    sp.sparray, sp.sparray, sp.sparray
]:
    if meas_ind is None:
        meas_ind = times
...
def find_alpha_complex_witheld(times: np.ndarray, measurements: np.ndarray, alpha0: float = 1, detail=False):
    h = 1e-6
    ...
    @lru_cache(10)
    def _loss_alpha(curr_alpha: complex) -> tuple[float, float]:
...
def find_alpha_generalized(
    ...
    """Find kalman parameter alpha using generalized method
    ...
    def v(a):
        x = sol(a[0])
        return (
            .5 * ((x.T @ H.T - z.T) @ Rinv @ (H @ x - z))
            + .5 * a * x.T @ G.T @ Qinv @ G @ x
        )
